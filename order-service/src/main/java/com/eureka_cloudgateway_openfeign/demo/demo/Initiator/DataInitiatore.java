...
public class DataInitiatore implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        if(event.getApplicationContext().getParent()==null){
            orderRepository.save(new Order(160, new Date(), Order.OrderState.CREATED));
            orderRepository.save(new Order(360, new Date(), Order.OrderState.CREATED));
            orderRepository.save(new Order(5500, new Date(), Order.OrderState.CREATED));
        }
    }
}
